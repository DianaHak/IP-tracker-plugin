...
class DIA_IPG_Ajax
{
  ...
  const NOTES_OPTION = 'dia_ipg_notes_rows';
  ...
  public static function init()
  {
    ...
    // Bulk actions
    add_action('wp_ajax_dia_ipg_bulk_action', [__CLASS__, 'bulk_action']);
  ...
  {
    $notes = [];
    $notes[] = ['ip' => '', 'comment' => 'Googlebot'];
    $notes[] = ['ip' => '', 'comment' => 'Bingbot'];
    $notes[] = ['ip' => '', 'comment' => 'Heavy bot'];
  ...
  {
    if ($table === 'top5m') return 5;
    if ($table === 'top1h') return 60;
    if ($table === 'top24') return 24 * 60;
    if ($table === 'top3d') return 72 * 60;
    if ($table === 'top7d') return 168 * 60;
    return 24 * 60;
  }
  ...
  private static function get_notes(): array
  {
    ...
    if (!is_array($notes) || count($notes) !== 30) {
  ...
  public static function bulk_action()
  {
    ...
    $do = isset($_POST['bulk_action']) ? sanitize_key((string) $_POST['bulk_action']) : '';
    $ips = isset($_POST['ips']) ? (array) $_POST['ips'] : [];

    $allowed = ['block_selected', 'unblock_selected', 'unblock_all'];
    if (!in_array($do, $allowed, true)) {
      wp_send_json_error(['message' => 'Invalid bulk action'], 400);
    }
    ...
    if ($do === 'unblock_all') {
      ...
      wp_send_json_success(['message' => 'All blocked IPs were unblocked.']);
    }
    ...
    if (empty($clean_ips)) {
      wp_send_json_error(['message' => 'No valid IPs selected.'], 400);
    }

    if ($do === 'block_selected') {
    ...
    if ($do === 'unblock_selected') {
      $blocked = array_values(array_filter($blocked, function ($x) use ($clean_ips) {
        return !in_array($x, $clean_ips, true);
      }));
    }

    DIA_IPG_Core::set_blocked_list($blocked);

    wp_send_json_success(['message' => 'Bulk action applied successfully.']);
  }
  ...
  public static function ajax_table()
  {
    ...
    if (!in_array($table, ['top5m', 'top1h', 'top24', 'top3d', 'top7d', 'recent'], true)) {
    ...
    if (in_array($table, ['top5m', 'top1h', 'top24', 'top3d', 'top7d'], true)) {

      $minutes = self::top_minutes_from_table($table);

      $title = 'Top IPs — last 24 hours';
      if ($table === 'top5m') $title = 'Top IPs — last 5 minutes';
      elseif ($table === 'top1h') $title = 'Top IPs — last 1 hour';
      elseif ($table === 'top3d') $title = 'Top IPs — last 3 days';
      elseif ($table === 'top7d') $title = 'Top IPs — last 7 days';

      if (!in_array($orderby, ['hits', 'last_seen'], true)) $orderby = 'hits';

      $countries = DIA_IPG_Logger::top_countries_minutes($minutes);

      $data = DIA_IPG_Logger::top_ips_paged_minutes(
        $minutes,
        $page,
        $per_page,
        $orderby,
      ...
      $recent_minutes = isset($_POST['recent_minutes']) ? (int) $_POST['recent_minutes'] : 0;
      if ($recent_minutes !== 5) $recent_minutes = 0;

      if ($recent_minutes === 5 && method_exists('DIA_IPG_Logger', 'recent_visits_paged_minutes')) {
  ...
  public static function notes_save()
  {
    ...
    if ($row < 0 || $row > 29) {
      wp_send_json_error(['message' => 'Invalid row'], 400);
    }
    ...
    $notes = self::get_notes();
    $notes[$row] = ['ip' => $ip, 'comment' => $comment];
  ...
  public static function export_csv()
  {
    ...
    $hours = 24;
    if ($table === 'top3d') $hours = 72;
    if ($table === 'top7d') $hours = 168;
    ...
    foreach ($all_rows as $r) {
      ...
      fputcsv($out, [$ip, $cc, $hits, DIA_IPG_Table::export_fmt_dt($last_seen)]);
    }
  ...
  public static function export_print()
  {
    ...
    $title = ($table === 'top24') ? 'Top IPs — last 24 hours' : (($table === 'top3d') ? 'Top IPs — last 3 days' : 'Top IPs — last 7 days');
